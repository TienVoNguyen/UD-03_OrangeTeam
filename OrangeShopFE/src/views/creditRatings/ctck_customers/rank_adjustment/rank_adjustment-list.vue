<template>
  <div class="app-container">
    ...
    <div class="row mt-3">
      <div class="col-4">
        <label for="type_score">Loại chấm điểm</label>
        <el-select id="type_score" value="" class="form-control non-border">
          <el-option v-for="item in type_scores" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </div>
      <div class="col-4">
        <label for="type_report">Loại BCTC năm gần nhất</label>
        <el-select id="type_report" value="" class="form-control non-border">
          <el-option v-for="item in type_reports" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </div>
      <div class="col-4">
        <label>Năm BCTC</label>
        <el-input id="year_bctc" v-model="temp.year_bctc" value="" class="form-control non-border" placeholder="Nhập năm BCTC" />
      </div>
    </div>
    <div class="row mt-5">
      <div class="col-6">
    ...
    <div class="row mt-3">
      ...
      >
        ...
        <el-table-column label="Loại chấm điểm">
          <template slot-scope="{row}">
            {{ row.type_score }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="Năm BCTC" align="center">
          <template slot-scope="{row}">
            {{ row.year_report }}
          </template>
        </el-table-column>
        <el-table-column label="Ngày tạo" width="160px" align="center">
          <template slot-scope="{row}">
            {{ row.date_created }}
          </template>
        </el-table-column>
...
import UpdateRank from '@/views/creditRatings/ctck_customers/rank_adjustment/update-rank_adjustment' // secondary package based on el-pagination
      ...
      this.list = [
        {
          ...
          type_score: 'Loại chấm điểm 1',
          type_report: 'Loại báo cáo 1',
          year_report: '2022',
          date_created: '06/05/2021'
        },
        {
          id: 2,
          code_tax: 'TAX012',
          ...
          type_score: 'Loại chấm điểm 2',
          type_report: 'Loại báo cáo 2',
          year_report: '2021',
        ...
        },
        {
          ...
          type_score: 'Loại chấm điểm 3',
          type_report: 'Loại báo cáo 3',
          year_report: '2023',
