<template>
  <div class="app-container">
    ...
    <div class="row mt-5">
      <div class="col-6">
        <button class="btn btn-outline-primary float-end" @click="searchModel()">Tìm kiếm</button>
      </div>
      <div class="col-6">
        <!-- <button class="btn btn-outline-warning" @click="handleCreate">Thêm mới</button> -->
        <router-link :to="{ path: '/creditRatingsRouter/ctck_customers/add_customer_identification'}" class="btn btn-outline-warning" style="color: #EF8C1A">Thêm mới</router-link>
      </div>
    </div>
    ...
    <div v-if="errorSearch.type!=-1" class="row mt-3">
      <span class="el-icon-warning" style="font-size: 40px;    color: #FF0000;    text-align: center;" />
      <span style="font-size: 20px;    color: #FF0000;    text-align: center;"> {{ errorSearch.message }}</span>
    </div>
    <div class="row mt-5">
      <el-table
        :key="tableKey"
      ...
      >
        <el-table-column label="Thao tác" width="100px" align="center">
          <template slot-scope="{row, $index}">
            ...
            <router-link :to="{ path: '/creditRatingsRouter/ctck_customers/update_customer_identification/'+row.id}" class="el-icon-edit" style="color: #083863" />
...
import DeleteCustomer from '@/views/creditRatings/ctck_customers/customer_identification/delete-customer_identification' // secondary package based on el-pagination
...
export default {
  ...
  methods: {
    ...
    searchModel() {
      this.checkValidateSearch()
    },
    checkValidateSearch() {
      if (
        (this.temp.code_tax == null || this.temp.code_tax === '' || this.temp.code_tax === undefined) &&
          (this.temp.code_reg == null || this.temp.code_reg === '' || this.temp.code_reg === undefined) &&
          (this.temp.code_cif == null || this.temp.code_cif === '' || this.temp.code_cif === undefined) &&
          (this.temp.code_cif_temp == null || this.temp.code_cif_temp === '' || this.temp.code_cif_temp === undefined)
      ) {
        this.errorSearch =
          {
            type: 1,
            message: 'Vui lòng nhập ít nhất 1 tham số trước khi tìm kiếm thông tin'
          }
      }
    },
    clearValidateSearch() {
