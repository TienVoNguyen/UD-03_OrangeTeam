<template>
  <ValidationObserver ref="observer">
    <el-dialog
      title="Import thông tin chức vụ"
      :visible.sync="dialogImportVisible"
      :before-close.sync="closeDialog"
    >
      <h3 slot="title" class="title mt-3"><strong>Import thông tin chức vụ</strong></h3>
      <el-form ref="dataForm" label-position="left" label-width="70px">
        ...
        >
          <el-form-item
            prop="file"
            :error="messageError(systemFields.code, errors[0])"
          >

            <label for="excel">Chọn file danh sách chức vụ<span class="text-danger">*</span></label>
      ...
      <div style="margin-left: 50px">
        <span>Tải file mẫu </span> <el-link style="margin-top: -4px;" type="primary" @click="doneImport()">tại đây</el-link>
        <div v-if="excelData.results.onsuccess">
          <span>Tổng số bản ghi hợp lệ {{ excelData.results.num_success }} / {{ excelData.results.total }}. Vui lòng tải kết quả Import </span> <el-link style="margin-top: -4px;" type="danger">tại đây</el-link>
        </div>
      </div>
